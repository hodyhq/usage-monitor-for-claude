...
- **Detail popup** (left-click) with a modern card layout: session and weekly limits as progress rings, dynamically detected per-model bars for all active quota types (Sonnet, Opus, Cowork, and any new quotas Anthropic adds) with [configurable field selection](docs/configuration.md#popup-fields), account info with plan chip, extra usage, reset countdowns, and a stale-data indicator when values may be outdated
- **Today's Tokens** - per-model token counts aggregated from your local Claude Code transcripts, covering models the usage API does not break out (like Fable 5) - fully offline, nothing leaves your machine
